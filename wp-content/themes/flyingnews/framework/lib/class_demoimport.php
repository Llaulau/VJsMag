...
class jwDemoImport {
    ...
    function __construct($filename = 'default') {
      // Load Importer API
        ...
        if (get_option('demo') == null) {
                add_option('demo', $filename);
                $type = "data";
        }else{
            $type = "preset";
        }
	...
	$this->fetch_attachments = true;
       ...
       // $this->id = 0;//(int) $_POST['import_id'];

                                      //$filename
	
	set_time_limit(0);
        
	$this->import( $filename ,$type);
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( $term_id ) {
				...
				if ( isset($cat['term_id']) )
					$this->processed_terms[intval($cat['term_id'])] = (int) $term_id;
                                        $this->_ids_cat[sizeof($this->_ids_cat)] = (int) $term_id;
				continue;
			}
			...
			$id = wp_insert_category( $catarr );
                        $this->_ids_cat[sizeof($this->_ids_cat)] = $id;
            ...
            // categories 
                ...
                if(isset($this->opt_categories[0])){                     
                    ...
                    foreach ($otp_cat_unserial as $key => $cat){
                        for ($i = 0; $i < sizeof($this->_ids_cat) ; $i++){
                            if($key == "category_".$this->categories[$i]['term_id']){                              
                                break;   
                            }
                        }
                        
                        
                        if(isset($this->_ids_cat[$i])){
                           $key = "category_".$this->_ids_cat[$i];
                           $opt_cat_new[$key] = $cat; 
                ...
                if(isset($this->opt_menus[0])){
                    
                    $cat = get_option( MENUS );
                    $cat['backup_log'] = date('r');
                    update_option(MENUS.BACKUPS, $cat);
                    
                    
                    $otp_menu_unserial = unserialize($this->opt_menus[0]);
                    foreach ($otp_menu_unserial as $key => $menu){
                        $opt_menu_new[$key] = $menu;
                        if($menu['menu_type'] == 'category'){
                            for ($i = 0; $i < sizeof($this->_ids_cat) ; $i++){
                                if($menu['menu_id'] == $this->categories[$i]['term_id']){             
                                    break;   
                                }
                            }
                            if($i < sizeof($this->_ids_cat)){
                            
                                foreach ((array) $this->_ids_nav_menu as $key_ids_nav_menu => $ids_nav_menu){
                                    if($ids_nav_menu == $this->_ids_cat[$i]){
                                        $opt_menu_new[$key_ids_nav_menu] = $menu;

                                        if(isset($this->_ids_cat[$i])){
                                             $opt_menu_new[$key_ids_nav_menu]['menu_id'] = (string) $this->_ids_cat[$i]; 
                    ...
                    update_option(MENUS, $opt_menu_new);
               }
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug', 'jawtemplates' );
			echo '<br />';
			return;
		}
		...
		$args = array(
			...
			'menu-item-status' => $item['status']
                ...
                if(isset($this->_ids_menus[$menu_id])){    
                    $id = wp_update_nav_menu_item( $this->_ids_menus[$menu_id], 0, $args );
    ...
    */
		...
		$widget_settings = $this->upload_widget_settings_file($file);
                //$upload = json_decode($string, true);
	...
	function parse( $file ) {
		...
		foreach ( $xml->xpath('/rss/channel/wp:term') as $term_arr ) {
			...
			$tl = $term_location_arr->children( $namespaces['wp'] );
                ...
                $xml_categories = $xml->xpath('/rss/channel/wp:flyingnews_categories');    
                $xml_menus = $xml->xpath('/rss/channel/wp:flyingnews_menus');
                $xml_options = $xml->xpath('/rss/channel/wp:flyingnews_options');
